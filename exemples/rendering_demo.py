...
def basic_rendering_demo():
    ...
    env, metadata = create_environment(
        "brix", render_mode="rgb_array", render_scale=8, color_scheme="classic"
    )
...
def color_scheme_demo():
    ...
    schemes = ["classic", "amber", "white", "blue", "retro"]
...
def batch_rendering_demo():
    ...
    grid_image = batch_render(displays_array, scale=3, color_scheme="classic")
...
def training_with_visualization():
    ...
    env, metadata = create_environment(
        "shooting-stars", render_mode="rgb_array", color_scheme="amber"
    )
